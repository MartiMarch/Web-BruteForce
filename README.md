...
<br>
<p align="justify">In conclusion, it's possible execute a brute force attack with Selenium, but it takes a lot of time. After a few searchs, i decide use the "mechanize" library. It also can be used to scrapping the web page and realize submit pettitions. After a changes o the code, it work better than selenium. Other important point is the wich number of threads use. If you select a lower number o threads it don't use all the parrallelitzon benefits, but if the number is higher it will increase the rescources of the pc without aport anything.</p>
